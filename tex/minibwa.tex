...
BWA-MEM3 is a new fork of BWA-MEM2 with speedup from the latency hiding technique and more frequent ungapped alignment to skip full dynamic programming.
...
BWA-MEM2 is 67\% faster than BWA-MEM and both BWA-MEME and BWA-MEM3 are 47\% faster than BWA-MEM2 (Fig.~\ref{fig:perf}a).
Minibwa is 284\% faster than BWA-MEM, 130\% faster than BWA-MEM2 and 57\% faster than BWA-MEME and BWA-MEM3.
The gap between minibwa and other BWA-MEM implementations is smaller for Hi-C and Roche's single-end SBX reads
perhaps because minibwa's new mate rescue heuristic (Section~\ref{sec:pe}) is disabled for these data types.
For long reads, minibwa is a little faster than minimap2 and is an order of magnitude faster than Winnowmap2.
For BS-seq short reads, minibwa also leads in performance.
...
Minibwa & 44,478 & 8,221 & 36,358 & 5,418 \\
\botrule
\end{tabular*}
\end{table}

We aligned HG002 short reads to GRCh38, called small variants with DeepVariant~\citep{Poplin:2018ab}
and compared the calls to the GIAB ground truth~\citep{Hansen2025.09.21.677443} with RTG vcfeval~\citep{Cleary023754}.
As is shown in Table~\ref{tab:var}, minibwa called slightly more false positives but fewer false negatives.
We inspected at the minibwa-specific false positive SNPs
and found they are mostly borderline cases that pass DeepVariant's internal filter in the minibwa alignment but are flagged as {\tt RefCall} in the BWA-MEM alignment.
