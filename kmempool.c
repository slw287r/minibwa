...
typedef struct {
	void *km; // kalloc memory manager (NULL for system malloc)
...
static void kmp_add_chunk(kmempool_t *mp) // add a new chunk
{
	...
	mp->chunk = Krealloc(mp->km, uint8_t*, mp->chunk, mp->n_chunk + 1);
	mp->p = mp->chunk[mp->n_chunk++] = Kcalloc(mp->km, uint8_t, x);
	mp->p_end = mp->p + x;
}
...
{
	return kmp_init3(0, sz, chunk_size);
}

void *kmp_init(unsigned sz) // fixed chunk size
{
	return kmp_init3(0, sz, 0x10000);
}
...
void kmp_destroy(void *mp_)
{
	...
	for (i = 0; i < mp->n_chunk; ++i) // free all chunks
		kfree(mp->km, mp->chunk[i]);
	kfree(mp->km, mp->chunk);
	kfree(mp->km, mp->buf);
	kfree(mp->km, mp);
}
...
void kmp_free(void *mp_, void *p)
{
	...
	if (mp->n_buf == mp->m_buf) { // then enlarge the buffer
		...
		mp->buf = Krealloc(mp->km, void*, mp->buf, mp->m_buf);
	}
	mp->buf[mp->n_buf++] = p;
}
